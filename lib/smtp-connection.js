...
const includes = require('node4mailer/lib/includes');
...
class SMTPConnection extends EventEmitter {
    constructor(server, socket) {
        ...
        this._parser.oncommand = function (/*...args*/) {
            return this._onCommand.apply(this, arguments);
        }.bind(this);
    ...
    _onCommand(command, callback) {
        ...
        if (!this.hostNameAppearsAs && commandName && includes(['MAIL', 'RCPT', 'DATA', 'AUTH'], commandName)) {
            this.send(503, 'Error: send ' + (this._server.options.lmtp ? 'LHLO' : 'HELO/EHLO') + ' first');
            return setImmediate(callback);
        }

        // Check if authentication is required
        if (!this.session.user && this._isSupported('AUTH') && includes(['MAIL', 'RCPT', 'DATA'], commandName) && !this._server.options.authOptional) {
    ...
    _isSupported(command) {
        ...
        return !includes(this._server.options.disabledCommands, command) &&
    ...
    handler_XCLIENT(command, callback) {
        ...
        for (let i = 0, len = parts.length; i < len; i++) {
            value = parts[i].split('=');
            key = value.shift();
            if (value.length !== 1 || !includes(allowedKeys, key.toUpperCase())) {
                this.send(501, 'Error: Bad command parameter syntax');
                return callback();
            }
            key = key.toUpperCase();
            ...
            if (includes(['[UNAVAILABLE]', '[TEMPUNAVAIL]'], value.toUpperCase())) {
                value = false;
            }
    ...
    handler_AUTH(command, callback) {
        ...
        if (!includes(this._server.options.authMethods, method) || typeof handler !== 'function') {
